...
class Result:
    def __init__(self, data=None):
        ...
        if data is not None:
            ...
            self.loop_count = int(data[9])
            self.drop_count = int(data[10])
            self.success_volume = int(data[11])
            self.fail_volume = int(data[12])
    ...
    def cdf(self, hop):
        return sum(self.hop_count[:hop + 1]) / self.sum_hops()
...
def generate_plots(data, name, location=None):
    ...
    N = len(range(2, 11))
    cgfp_bgp = tuple([data['CGFP-BGP'].cdf(i) for i in range(2, 11)])
    cgc_bgp = tuple([data['CGC-BGP'].cdf(i) for i in range(2, 11)])
    sfp = tuple([data['SFP'].cdf(i) for i in range(2, 11)])
    ...
    ax.set_xticklabels(tuple(range(2, 11)))
    ax.set_ylim([0, 1])
...
if __name__ == '__main__':
    ...
    location = 231
    final = dict()
    for filename in [BLACKHOLE_3_3, BLACKHOLE_5_5, DEFLECTION_3_3, DEFLECTION_5_5, BOTH_3_3, BOTH_5_5]:
        final[filename] = dict()
